...
export default async function BookDetailsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const bookDetails = await DatabaseClient.getOneRecordById<BookRecord>(DATABASE_CONFIG.tables.books.id, id);
            ...
            <div className="mt-6">
                <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700 rounded-lg border border-zinc-200 dark:border-zinc-800 overflow-hidden">
                    <tbody className="bg-white dark:bg-zinc-900 divide-y divide-zinc-200 dark:divide-zinc-700">
                        <tr>
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-zinc-900 dark:text-white">العميل</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-zinc-600 dark:text-zinc-400">{bookDetails?.client || "—"}</td>
                        </tr>
                        <tr>
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-zinc-900 dark:text-white">تاريخ الرفع</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-zinc-600 dark:text-zinc-400">{bookDetails?.upload_date || "—"}</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-zinc-900 dark:text-white">إجمالي الإيرادات</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-zinc-600 dark:text-zinc-400">
                                {bookDetails?.total_revenue !== undefined
                                    ? `$${bookDetails.total_revenue.toFixed(2)}`
                                    : "—"}
                            </td>
                        </tr>
                    </tbody>
